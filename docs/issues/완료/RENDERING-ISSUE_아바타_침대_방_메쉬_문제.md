...
2. **침대 거리 문제**: 침대가 아바타에 너무 가까이 위치함
...
- 침대가 너무 가까이 위치하여 전체적인 비율이 맞지 않음

## Investigation Notes

### Code Flow

1. **`src/renderer/scene/index.ts`** - Scene 및 침대 설정
   - 침대 위치: `(0, 1.4, 4.5)` looking at `(0, 0.6, 0)`
   - 침대 frustum 설정이 아바타 크기와 맞지 않음
...
2. **침대 설정 조정**:
   - 침대 위치 변경: `(0, 1.4, 5.0)` 또는 더 먼 위치로 테스트
   - 침대 frustum 설정 확인 (near/far plane)
...
4. **씬 구성 확인**:
   ...
   - 침대 앵글 조정 (lookAt 위치 변경)

## Files to Check

- `src/renderer/scene/index.ts` - 침대 및 씬 설정
...
2. **침대 위치 테스트**:
   - `src/renderer/scene/index.ts`에서 침대 위치 변경: `(0, 1.4, 5.0)`
